...
lm_school = school |> select(c(`Mean Scale Score_e`, `Mean Scale Score_m`, `% Poverty`, `% ELL`, `% SWD`, `ENI`, `Diversity`))
english_reg = lm(`Mean Scale Score_e` ~ `% Poverty` + `% ELL` + `% SWD` + `ENI` + `Diversity`, data = lm_school)
summary(english_reg)

math_reg = lm(`Mean Scale Score_m` ~ `% Poverty` + `% ELL` + `% SWD` + `ENI` + `Diversity`, data = lm_school)
summary(math_reg)
